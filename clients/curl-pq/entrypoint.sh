...
while true; do
  ...
  # --curves sets the key share groups; curl passes them to OpenSSL.
  ...
  # curl verbose output: "* SSL connection using TLSv1.3 / TLS_AES_256_GCM_SHA384 / X25519MLKEM768 / ..."
  ...
  ERR_RAW=$(echo "$VERBOSE" | grep -i "curl:.*error\|failed\|could not" | head -1 | tr '"' "'" || true)
  if [ -n "$ERR_RAW" ]; then
    ERR_FIELD="\"${ERR_RAW}\""
  else
    ERR_FIELD="null"
  ...
  printf '{"client":"curl-pq","ts":"%s","curve":"%s","hrr":%s,"latency_ms":%s,"error":%s}\n' \
